...
* 3\. [PaaS-TA 5.0 설치](#108)
    ...
    * [PaaS-TA 설치 Shell Scripts](#1021)
        *  [deploy-aws.sh](#1022)
        *  [deploy-azure.sh](#1023)
        *  [deploy-gcp.sh](#1024)
        *  [deploy-openstack.sh](#1025)
        *  [deploy-vsphere.sh](#1026)
        *  [deploy-bosh-lite.sh](#1027)
        *  [PaaS-TA Operation 파일](#1028)
        *  [common_vars.yml](#1029)
        *  [{IaaS}-vars.yml](#1030)
        *  [PaaS-TA 그외 Variable List](#1031)
    * [PaaS-TA 설치](#1032)
    * [PaaS-TA 로그인](#1033)
    * [통합 Monitoring을 적용한 PaaS-TA 5.0 설치](#1034)
...
$ git clone https://github.com/okpc579/PaaS-TA-Deployment.git paasta-deployment
※ ${HOME} = /home/${inception_username}
```
- release, stemcell을 PaaS-TA 사이트에서 내려받아 정해진 경로에 복사한다.(선택)
...
압축을 풀면 아래와 같이 ${HOME}/workspace/paasta-5.0/release/paasta 디렉터리가 생성되며 릴리즈 파일(tgz)이 존재한다.
...
PaaS-TA는 Ubuntu xenial stemcell 315.64를 기반으로 한다. Stemcell은 배포 시 생성되는 PaaS-TA VM Base OS Image이다.  
...
```
$ bosh –e {director_name} update-cloud-config ${HOME}/workspace/paasta-5.0/deployment/paasta-deployment/cloud-config/{iaas}_cloud_config.yml
```
...
```
$ chmod +x ${HOME}/workspace/paasta-5.0/deployment/p-deployment/update-runtime-config.sh
```
...
```
### <div id='1023'/>3.7.2. deploy-azure.sh
```
bosh -e {director_name} -d paasta -n deploy paasta-deployment.yml \	# PaaS-TA Manifest File
...
### <div id='1024'/>3.7.3. deploy-gcp.sh
```
...
### <div id='1025'/>3.7.4. deploy-openstack.sh
```
bosh -e {director_name} -d paasta -n deploy paasta-deployment.yml \	# PaaS-TA Manifest File
...
system_domain: 61.252.53.246.xip.io	                # Domain (xip.io를 사용하는 경우 HAProxy Public_IP와 동일)
...
### <div id='1031'/>3.7.10. PaaS-TA 그외 Variable List
...
### <div id='1034'/> * 통합 Monitoring을 적용한 PaaS-TA 5.0 설치

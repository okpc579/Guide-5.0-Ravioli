...
* 3\. [BOSH 설치 환경 구성 및 설치](#109)
    ...
    * [BOSH 설치](#1012)
        ...
        *  [BOSH 환경 설정](#1017)
            *  [deploy-aws.sh](#1018)
            *  [deploy-azure.sh](#1029)
            *  [deploy-gcp.sh](#1020)
            *  [deploy-openstack.sh](#1021)
            *  [deploy-vsphere.sh](#1022)
            *  [deploy-bosh-lite.sh](#1023)
            *  [BOSH Optional 파일](#1024)
            *  [PaaS-TA Monitoring Operation 파일](#1025)
            *  [common_vars.yml](#1026)
            *  [aws-vars.yml](#1027)
            *  [azure-vars.yml](#1028)
            *  [gcp-vars.yml](#1029)
            *  [openstack-vars.yml](#1030)
            *  [vsphere-vars.yml](#1031)
            *  [bosh-lite-vars.yml](#1032)
        *  [BOSH 설치](#1033)
        *  [BOSH 로그인](#1034)
        *  [CredHub](#1035)
            *  [CredHub CLI 설치](#1036)
...
BOSH2 CLI 6.0 이하 버전 사용 시, 인증서 기간을 늘리고 싶다면 BOSH CLI 소스를 다운로드해 컴파일하여 사용해야 한다. 소스 컴파일 방법은 다음 가이드를 참고한다.  
...
- ※ 릴리즈는 배포시 compiled-releases 옵션파일을 빼면, 스템셀은 BOSH 명령어로 인터넷이 연결되어 있다면 다운로드가 가능하기 때문에 필수는 아니나, 설치속도는 느릴 수 있다.
...
$ cd ${HOME}/workspace/paasta-5.0/deployment
$ ls
common  monitoring-deployment  paasta-deployment  portal-deployment  service-deployment
```

<table>
<tr>
<td>common</td>
<td>PaaS-TA 및 각종 Service 설치를 위한 공통 변수가 들어있는 manifest가 존재하는 디렉터리</td>
</tr>
<tr>
<td>monitoring-deployment</td>
<td>PaaS-TA Monitoring 설치를 위한 manifest 및 설치 파일이 존재하는 디렉터리</td>
</tr>
<tr>
<td>paasta-deployment</td>
<td>BOSH와 PaaS-TA 설치를 위한 manifest 및 설치 파일이 존재하는 디렉터리</td>
</tr>
<tr>
...
```
$ cd ${HOME}/workspace/paasta-5.0/release
$ ls
...
```
$ cd ${HOME}/workspace/paasta-5.0/stemcell
$ ls
...
#### <div id='1018'/>3.3.5.1. deploy-aws.sh
...
```
#### <div id='1019'/>3.3.5.2. deploy-azure.sh
```
bosh create-env bosh.yml \                         
...
#### <div id='1020'/>3.3.5.3. deploy-gcp.sh
```
bosh create-env bosh.yml \                         
...
#### <div id='1021'/>3.3.5.4. deploy-openstack.sh
...
#### <div id='1022'/>3.3.5.5. deploy-vsphere.sh
...
#### <div id='1023'/>3.3.5.6. deploy-bosh-lite.sh
```
bosh create-env bosh.yml \
...
#### <div id='1024'/>3.5.5.7. BOSH Optional 파일
...
#### <div id='1025'/>3.3.5.8. PaaS-TA Monitoring Operation 파일
...
#### <div id='1026'/>3.3.5.9. common_vars.yml
...
#### <div id='1027'/>3.3.5.10. aws-vars.yml
...
#### <div id='1028'/>3.3.5.11. azure-vars.yml
```
# BOSH VARIABLE
bosh_client_admin_id: "admin"				# Bosh Client Admin ID
private_cidr: "10.0.1.0/24"				# Private IP Range
...
#### <div id='1029'/>3.3.5.12. gcp-vars.yml
```
# BOSH VARIABLE
bosh_client_admin_id: "admin"		# Bosh Client Admin ID
inception_os_user_name: "ubuntu"	# Home User Name
...
#### <div id='1030'/>3.3.5.13. openstack-vars.yml
...
#### <div id='1031'/>3.3.5.14. vsphere-vars.yml
...
#### <div id='1032'/>3.3.5.15. bosh-lite-vars.yml
```
# BOSH VARIABLE
bosh_client_admin_id: "admin"		# Bosh Client Admin ID
inception_os_user_name: 'ubuntu'	# Home User Name
...
#### <div id='1036'/>3.3.8.1. CredHub CLI 설치
...
#### <div id='1037'/>3.3.8.2. CredHub 로그인
